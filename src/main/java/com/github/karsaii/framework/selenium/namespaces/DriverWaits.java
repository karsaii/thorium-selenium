...
public interface DriverWaits {
    static DriverFunction<Boolean> waitNavigatedTo(String url, String query, int interval, int timeout) {
        return ifDriver(
                ...
                driver -> Wait.core(new WaitData<>(ExpectedConditions.isUrlContains(url, query), WaitPredicateFunctions::isTruthyData, "Waiting for url", WaitTimeDataFactory.getWithDefaultClock(interval, timeout))).apply(driver),
